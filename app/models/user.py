...
class UserTable(Base, SQLAlchemyBaseUserTable):
    id = Column(_type.Integer, primary_key=True)
    username = Column(_type.String(length=50), unique=True, index=True, nullable=False)
    email = Column(_type.String(length=320), unique=True, index=True)
